...
async def search_term(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        for i, (term, data, score, channel_name, channel_id) in enumerate(unique_results, 1):
            # CRITICAL FIX: Ensure the data structure is a dictionary before trying to access properties
            if not isinstance(data, dict):
                logger.error(f"Skipping corrupted result data in search_term for term: {term}")
                continue
...
async def stats(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if default_knowledge:
            ...
            # CRITICAL FIX: Add isinstance check to safely count definitions
            total_definitions += sum(
                len(data.get("definitions", [data.get("definition", "")]))
                for data in default_knowledge.values() if isinstance(data, dict)
            )
        ...
        if knowledge_dir.exists():
            ...
            for kb_file in channel_files:
                try:
                    ...
                    total_definitions += sum(
                        len(data.get("definitions", [data.get("definition", "")]))
                        for data in knowledge.values() if isinstance(data, dict)
                    )
                except Exception as e:
                    logger.error(f"Error processing {kb_file}: {e}")
